...
echo $PHP_CONFIG_FLAGS|grep -q libgd && rpmbuild --define "_topdir `pwd`"  -ba SPECS/gd.spec > ${LOGFILE}
...
rpmbuild --define "_topdir `pwd`" ${PHP_CONFIG_FLAGS} -ba SPECS/php56.spec > ${LOGFILE}
