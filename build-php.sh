...
echo $PHP_CONFIG_FLAGS|grep -q httpd24 && [ ! -z ${APACHE_VERSION} ] && \
  yum -y --nogpgcheck install RPMS/$(uname -m)/httpd-{,devel-}${APACHE_VERSION}*.$(uname -m).rpm && \
  cp SOURCES/httpd-php-build.conf ${DISTRO}/etc/httpd/conf/httpd.conf

echo $PHP_CONFIG_FLAGS|grep -q libgd && ${CMD_BUILD_DEP} -y SPECS/gd.spec
...
echo $PHP_CONFIG_FLAGS|grep -q libgd && yum -y --nogpgcheck install RPMS/$(uname -m)/gd-last-{,devel-}${GD_VERSION}*.$(uname -m).rpm 

spectool -g -C SOURCES SPECS/php56.spec
${CMD_BUILD_DEP} -y SPECS/php56.spec
