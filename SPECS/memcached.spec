...
Provides: memcached = %{version}-%{release}
Provides: memcached%{?_isa} = %{version}-%{release}
Provides: config(memcached) = %{version}-%{release}
Conflicts: memcached < %{version}
...
touch -r %{SOURCE1} %{buildroot}/%{_sysconfdir}/sysconfig/memcached
...
if [ "$1" -eq "0" ] ; then
    ...
    chkconfig --del memcached &> /dev/null
fi
%endif # with_systemd
...
%else
if [ "$1" -ge "1" ]; then
    service memcached condrestart &> /dev/null
fi
%endif # with_systemd
...
%changelog
* Mon Dec 19 2016 Gregory Boddin <gregory@siwhine.net> - 1.4.24-1
- Import from IUS
- Dropped custom name
